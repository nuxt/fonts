...
Nuxt Fonts works with the Vite, webpack and rspack builders. The behaviour is the same in each case: we inject `@font-face` rules into your stylesheets, download the fonts at build time and serve them from `/_fonts` with long-lived cache headers.
