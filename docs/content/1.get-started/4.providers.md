...
By default the stylesheet shipped with the package is read from disk (`remote: false`) rather than from a CDN, and the font files it references are read from the installed package and served from your own origin.
