...
Google Fonts serves a font file containing only the axes you request, so a font is only variable to the extent you ask for it. Requesting a single weight (the default) returns a static font file, and requesting a weight range returns a font that is variable on `wght` alone.

This means `font-variation-settings` and `font-stretch` in your CSS have no effect unless the corresponding axis was requested. To vary weight, ask for a range:
...
export default defineNuxtConfig({
  ...
  fonts: {
    families: [
      { name: 'Archivo', provider: 'google', weights: ['100 900'] },
    ],
  },
})
```

To vary any other axis (such as `wdth` for the 'condensed' and 'expanded' variants of a family), request it explicitly:
    ...
    families: [
      {
        ...
        providerOptions: {
          google: {
            experimental: {
              variableAxis: {
                // a range, or a list of single values, per axis
                wdth: [['62', '125']],
              },
            },
          },
...
The generated `@font-face` will then include `font-weight: 100 900` and `font-stretch: 62% 125%`, and `font-stretch: expanded` or `font-variation-settings: 'wdth' 125` will work as it does on [fonts.google.com](https://fonts.google.com/).

::warning
Requested ranges must be within the axis ranges the family actually supports (shown on the family's page on [fonts.google.com](https://fonts.google.com/)). Google Fonts rejects out-of-range requests, and the font will not be resolved.
::
