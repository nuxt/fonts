...
The [`local` provider](/get-started/providers#local) also accepts named weights, in either the hyphenated or unhyphenated spelling, so `weights: ['medium', 'semibold']` is equivalent to `weights: [500, 600]`. Remote providers expect numeric weights.
...
Chooses the values a variable font is shipped at, for every family. Each OpenType axis tag takes an array of values: a number or string, such as `CASL: [1]`, pins the axis to a single value, and a `[min, max]` pair or `{ min, max }` object, such as `MONO: [{ min: 0, max: 1 }]`, narrows it to an inclusive range. This can be overridden for a single font with [`families`](#families).
...
Providers that can instance a font server-side, such as Google Fonts, serve the file already resolved at those values. Otherwise the axis is applied to the downloaded file with [`subset-font`](https://github.com/papandreou/subset-font), which needs a glyph list to subset to: either the family's `glyphs`, or the characters the face's `unicode-range` declares. Without either, the file is emitted untouched and a `font-variation-settings` descriptor renders the font at the requested values instead.

`wght` and `ital` are left alone, because `@font-face` descriptors already express them.

::warning
Applying an axis is a subsetting pass, so it modifies the font files you ship. Check that the licence of each font allows it.
::
...
Set `preload: true` to preload a single font face for every family. It picks a face from the first configured [`subsets`](#subsets) entry (otherwise one covering Basic Latin), prefers upright over italic, and then prefers the weight closest to 400.

Pass an object with any of `subsets`, `styles` and `weights` to preload every font face that matches all of the given values.
...
export default defineNuxtConfig({
  ...
  fonts: {
    defaults: {
      // preload only the matching font faces
      preload: { subsets: ['latin'], styles: ['normal'], weights: [400] },
      // or preload one upright font face near weight 400 in the first configured subset (or Basic Latin)
      // preload: true,
      // or opt out entirely
      // preload: false,
...
Defines whether to inject `@font-face` regardless of usage in project. This is the escape hatch for fonts Nuxt Fonts can't detect by scanning your CSS: see [when a font isn't picked up](/get-started/usage#when-a-font-isnt-picked-up).
...
Default: *inherited from [`defaults.variableAxis`](#defaults)*
...
In a Vite production build, `prefix` names a directory inside `app.buildAssetsDir`, so the default resolves to `/_nuxt/fonts`. In development, and with the webpack and rspack builders, it is a public path and defaults to `/_fonts`. See [where fonts are served from](/advanced#where-fonts-are-served-from).
