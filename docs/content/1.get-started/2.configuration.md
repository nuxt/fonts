...
Chooses the values a variable font is shipped at, keyed by OpenType axis tag, for every family. A number or string pins an axis to a single value, and a `[min, max]` pair or `{ min, max }` object narrows it to an inclusive range. Can be overridden for a single font with [`families`](#families).
...
Chooses the values this family's variable font is shipped at, keyed by OpenType axis tag.
